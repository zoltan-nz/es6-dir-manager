...
//     // console.log(fileAbsolutePath, stats.isDirectory(), humanFormat(stats.size), humanFormat(totalBytes));
//
...
async function calculateSize(absolutePath, depth) {
  ...
  await files.forEach(async (file) => {
    ...
    let stats;
    try {
      stats = await fs.stat(fileAbsolutePath);
    } catch (e) {
    ...
    if (stats.isDirectory()) {
      const children = await calculateSize(fileAbsolutePath, localDepth);
      localTotalBytes += children.size;
      items += children.items;
    }
  });
